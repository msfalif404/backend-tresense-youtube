...
from .youtube_list_api import get_random_video_ids_from_indonesia
...
def oauth2callback(code: str):
    ...
    return HTMLResponse(content=f"Login berhasil! Sekarang kunjungi /scrape_all, dengan akses token: {user_tokens['access_token']}")
...
def scrape_all():
    ...
    video_ids = get_random_video_ids_from_indonesia(max_channels=12, videos_per_channel=12)

    all_results = []

    for video_id in video_ids:
        metadata = get_video_metadata(video_id, access_token)
    ...
    return {"message": "Komentar dan metadata berhasil disimpan ke comments_with_replies.json"}
